...
from jbfilter import load_depth_map, refine
from SAM import setup_sam_model, create_guidance_image_from_sam
from sam2.build_sam import build_sam2
from visualize import visualize_comparison

sys.path.append ('Depth-Anything-V2/metric_depth') #(r"C:\Users\Startklaar\PycharmProjects\pythonProject3\Depth-Anything-V2\metric_depth")
...
def main():
    ...
    parser.add_argument('--load-from', default='metric_depth/depth_anything_v2_metric_hypersim_vitb.pth', type=str,
                        required=False,
                        help='Path to the pre-trained model weights.')
    parser.add_argument('--max-depth', default=20, type=float,
                        help='Maximum depth value for the depth map.')
    parser.add_argument('--img-path', default='Images/chair/Image 2.jpg',
    ...
    parser.add_argument('--focal-length-x', default=635.64, type=float,
                        help='Focal length along the x-axis.')
    parser.add_argument('--focal-length-y', default=636.85, type=float,
    ...
    for k, filename in enumerate(filenames):
        ...
        # raw_depth = load_depth_map(resize_pred)
        
        guide, edge_map = create_guidance_image_from_sam(image, sam_model)
        depth_array_float32 = depth_array.astype(np.float32)
        
        refined_depth = refine(guide, depth_array_float32, d=9, sigmaColor=75, sigmaSpace=75)
        visualize_comparison(depth_array, refined_depth, edge_map, guide)
        #'''
        ...
        z = refined_depth # np.array(resized_pred)
